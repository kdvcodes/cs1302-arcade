...
public class TetrisTile extends Tile<Tetrimino> {
	...
	final PixelReader curtain = new Image("/tetris/curtain.png")
			.getPixelReader();
	 ...
	 */
	public TetrisTile(int row, int column, int xStart, int yStart,
			Tetris game) {
		super(row, column, xStart, yStart, size, offset, game);
	}
		...
		for (int x = 0; x < size; x++) {
			for (int y = 0; y < size; y++) {
				if (s.image().getArgb(x, y) == 0xFFec1c24) {
					img.getPixelWriter().setArgb(x, y,
							s.getColor(((Tetris) game).getLevel()));
				} else {
		...
		for (int x = 0; x < size; x++) {
			for (int y = 0; y < size; y++) {
				if (curtain.getArgb(x, y) == 0xFF3f48cc) {
					img.getPixelWriter().setArgb(x, y,
							Shape.getColor2(((Tetris) game).getLevel()));
				}
				if (curtain.getArgb(x, y) == 0xFFec1c24) {
					img.getPixelWriter().setArgb(x, y,
							Shape.getColor1(((Tetris) game).getLevel()));
				} else {
