...
/**
 * This is the tetris board class that holds most of the logics for the tetris
 * game
 */
public class TetrisBoard extends Board {
	 ...
	 * 
	 * @param rows    rows val
	 * @param columns cols val
	 * @param game    current game
	 */
	public TetrisBoard(int rows, int columns, Tetris game) {
		this.game = game;
		playField = new TetrisTile[rows][columns];
		clearAnim = new Timeline(
				new KeyFrame(Duration.millis(67), this::clear1));
		clearAnim.setCycleCount(6);
		clearAnim.setOnFinished(this::clear2);
		curtain = new Timeline(
				new KeyFrame(Duration.millis(67), this::drawCurtain));
	 ...
	 * 
	 * @param rows    rows val
	 * @param columns col val
	 * @param game    current game
	 * @param xStart  xStart val
	 * @param yStart  yStart val
	 */
	public TetrisBoard(int rows, int columns, Tetris game, int xStart,
			int yStart) {
		playField = new TetrisTile[rows][columns];
		this.game = game;
		for (int i = 0; i < playField.length; i++) {
